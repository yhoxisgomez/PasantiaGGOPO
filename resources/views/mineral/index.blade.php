...
@section('content')
  ...
  <a href="{{url('mineral/create')}}"class="btn btn-app"><i class="fa fa-plus-circle"></i>Registrar
  </a>
  <div class="card">
  <div class="card-body table-responsive{-sm|-md|-lg|-xl}">
<center><h2>Mineral</h2></center>
       <table id="mineral" class="table table-responsive-sm table-striped table-bordered" style="text-align:center;">
        ...
        <thead class="import">
      		...
      		<tbody style="background:rgba(255, 255, 255, 0.829);">
      			@foreach ($mineral as $c)
      			  <tr>

                    <td>{{$c->tipo_mineral}}</td>
      				<td>{{$c->nombre}}</td>

      				<td>

      		        <a class="btn btn-app" href="{{url('/mineral/'.$c->id.'/edit')}}"><i class="fa fa-edit"></i>Editar
                    </a>

      				<form method="post" action="{{url('/mineral/'.$c->id)}}" style="display:inline">
      					{{csrf_field()}}
                        {{method_field('DELETE')}}
